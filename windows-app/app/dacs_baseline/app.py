...
def _python_exe(root: Path) -> Path:
    ...
    if venv.exists():
        return venv
    # windows-app layout: .venv is one level above app/
    parent_venv = root.parent / ".venv" / "Scripts" / "python.exe"
...
class App(tk.Tk):
    ...
    def _build_args(self) -> list[str]:
        ...
        if frozen:
            # Same exe; launcher forwards "scan ..." to CLI
            args = [py, "scan"]
        else:
            args = [py, "-m", "dacs_baseline", "scan"]
        args.extend([
            "--label",
